...
そこで、Help:Contents/jaページで右上の▼マークから、「移動」→ 新しいページ名に「**ヘルプ**」(セレクトボックス)と「**目次**」(フリー入力)と入力し、「ページを移動する」をクリック。
これでツールバーのリンクからヘルプ一覧へアクセスできるようになる。
...
ここでは、MediaWiki を `/var/www/html/wiki` に導入し、URLから見られるところまで完了した、という前提で解説する。
...
![](/img/blog/2012-29/20121230185733.jpeg)
...
これで数式が見られるようになっているはず。試しに以下を入力してみると出力が可能になるはずだ。
